...
public class SplitStyling: ObservableObject {
  ...
  /// The visible thickness of the default Splitter and the `spacing` between the
...
public init(color: Color? = nil,
    ...
    self.previewHide = false
  }
  
  /// As an ObservableObject, when we want to change to a different SplitStyling, we need
  /// to just modify the properties of this instance.
  public func reset(from styling: SplitStyling) {
    self.color = styling.color
    self.inset = styling.inset
    self.visibleThickness = styling.visibleThickness
    self.invisibleThickness = styling.invisibleThickness
    self.hideSplitter = styling.hideSplitter
    self.previewHide = styling.previewHide
  }
}
